...
void Control::census(float **imgin, int i, int j, bool *tab1)
{
    ...
    if (imgMatrix[i][j] <= imgMatrix[i][j - 1])
        tab1[0] = 1;
    else
        tab1[0] = 0;
    if (imgMatrix[i][j] <= imgMatrix[i + 1][j - 1])
        tab1[1] = 1;
    else
        tab1[1] = 0;
    if (imgMatrix[i][j] <= imgMatrix[i + 1][j])
        tab1[2] = 1;
    else
        tab1[2] = 0;
    if (imgMatrix[i][j] <= imgMatrix[i + 1][j + 1])
        ...
        for (int j = 0; j < 100; j++)
        {
            ...
            for (int k = 0; k < 3; k++)
            {
                for (int l = 0; l < 3; l++)
                {
                    if (element[k][l])
                    {
                        if (copyimg[(i - 1 + k) < 0 ? 0 : (i - 1 + k) > 99 ? 99
                                                                           : i - 1 + k][(j - 1 + l) < 0 ? 0 : (j - 1 + l) > 99 ? 99
                                                                                                                               : j - 1 + l] > 128)
                        {
                            domorf = true;
                        }
